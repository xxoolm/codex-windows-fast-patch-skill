...
- When sharing manual GitHub install instructions, do not recommend a generic `Copy-Item -Recurse . ...` command. It can copy the caller's whole current workspace if they run it from the wrong directory. Use a repository-root check and copy only `SKILL.md`, `agents`, and `scripts`.
