...
const mobileSetupFile = findJs("codex mobile setup dialog bundle", (file, text) =>
  ...
  text.includes("async function Me") &&
...
function patchFlowHelpers(text) {
  ...
  const anchor = "var zX=`app_EMoamEEZ73f0CkXaXp7hrann`";
...
function patchDesktopFetch(text) {
  ...
  const next = replaceExact(text, oldAnchor, newAnchor, "desktop_fetch Authorization");
...
function patchAppServerAuthFallback(text) {
  ...
  if (!next.includes(marker)) {
    ...
    next = replaceExact(next, oldBh, newBh, "remote-control app-server auth fallback Bh");
    status = "patched";
  }
...
function patchStepUpFlow(text) {
  ...
  if (!next.includes(yxMarker)) {
    next = replaceExact(next, oldYx, newYx, "remote-control step-up YX");
    status = "patched";
  }
  if (!next.includes(tzMarker)) {
    next = replaceExact(next, oldTz, newTz, "remote-control step-up token exchange");
    status = "patched";
  }
  return { text: next, status };
}
...
function patchRemoteControlHttp(text) {
  ...
  return { text: replaceExact(text, oldNg, newNg, "remote-control HTTP diagnostics"), status: "patched" };
}
...
function patchRemoteControlAuthorize(text) {
  ...
  return { text: replaceExact(text, oldBg, newBg, "remote-control authorize flow"), status: "patched" };
}
...
function patchDeviceKeyCreationLogs(text) {
  ...
  return { text: replaceExact(text, oldCreate, newCreate, "remote-control device key creation logs"), status: "patched" };
}
...
function patchSoftwareDeviceKeyFallback(text) {
  ...
  return { text: replaceExact(text, oldWz, newWz, "remote-control software device-key fallback"), status: "patched" };
}
...
function patchMobileSetup(text) {
  ...
  const next = replaceExact(text, oldUi, newUi, "mobile setup 401 login redirect");
  if (next.includes(oldUi) || !next.includes(marker)) {
...
function patchRemoteConnectionsSettingsVisibility(text) {
  ...
  const next = replaceExact(text, oldVisibility, newVisibility, "remote connections local setup visibility");
